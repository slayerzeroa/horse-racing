...
data = pd.read_csv('C:/Users/slaye/PycharmProjects/Horse_Racing/data/datasets/test/seoul_horse_records.csv')
# ord == 1 -> 1 else 0 (1위만 1 나머지 0)
data = data[data['ord']<10]
data['ord'] = data['ord'].apply(lambda x: 1 if x==1 else 0)

data = data.drop(['jkName', 'rcNo', 'rcTime', 'speed', 'rcTime_mean', 'weather'], axis=1)
...
print(data['rcDate_diff'].iloc[1000])
...
classifier = XGBClassifier(n_estimators=400000, random_state=0, max_depth=4, learning_rate=0.01, min_child_weight=5, gamma=0.1, subsample=0.8, colsample_bytree=0.8, reg_alpha=0.005)
